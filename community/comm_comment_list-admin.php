...
$t_sql = "SELECT COUNT(1) FROM comm_comment";
...
// 假設默認排序是按 comm_comment_id 升序
$sortColumn = $_GET['sort'] ?? 'comm_comment_id';
$order = $_GET['order'] ?? 'ASC';

// 確保只允許特定的列名和排序方向
$allowedSortColumns = ['comm_comment_id', 'context', 'status', 'created_at', 'updated_at', 'post_id', 'user_id'];
$allowedOrder = ['ASC', 'DESC'];
$sortColumn = in_array($sortColumn, $allowedSortColumns) ? $sortColumn : 'comm_comment_id';
...
if ($totalRows > 0) {
    ...
    // 修改 SQL 查詢以包含排序參數
    $sql = sprintf(
        "SELECT c.*, u.username FROM comm_comment AS c 
            ...
            <div class="row align-items-center">
                ...
                <div class="col-auto me-3">
                    <div class="input-group">
                        ...
                        <!-- Return button  -->
                        <div class="col-auto">
                            <a href="comm_comment_list.php" class="btn btn-primary mb-3"><i class="fa-solid fa-rotate-left"></i></a>
                        </div>
                        <!-- Return button  -->
                    </div>
                </div>
                ...
                <div class="col-auto">
                    ...
                    <a href="comm_comment_add.php" class="btn btn-primary mb-3"><i class="fa-solid fa-plus"></i></a>
                ...
                <table class="table table-bordered table-striped table-primary">
                        ...
                        <tr>
                            ...
                            <th>
                                <div style="display: flex; align-items: center; justify-content: space-between;">
                                    <span>comm_comment_id</span>
                                    <a href="?sort=comm_comment_id&order=<?= $order === 'ASC' ? 'DESC' : 'ASC' ?>">
                    ...
                    <tbody>
                        <?php foreach ($rows as $row) : ?>
                            <tr>
                                <td>
                                    <a href="javascript: delete_one(<?= $row['comm_comment_id'] ?>)">
                                        <i class="fa-solid fa-trash-can"></i>
                                    </a>
                                </td>
                                <td><?= $row['comm_comment_id'] ?></td>
                                <td><?= $row['context'] ?></td>
                                <td><?= $row['status'] ?></td>
                                ...
                                <!-- 避免 XSS 攻擊問題 -->
                                <td>
                                    <a href="comm_comment_edit.php?comm_comment_id=<?= $row['comm_comment_id'] ?>">
...
<script>
    function delete_one(comm_comment_id) {
        if (confirm(`Do you want to delete the data with the ID ${comm_comment_id} ?`)) {
            location.href = `comm_comment_delete.php?comm_comment_id=${comm_comment_id}`
        }
    }
    ...
    function searchData() {
        ...
        fetch(`comm_comment_search-api.php?keyword=${keyword}`)
            .then(r => r.json())
            .then(result => {
