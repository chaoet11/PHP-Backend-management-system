...
$sql = "SELECT * FROM bar_type WHERE bar_type_id=$bar_type_id";

$row = $pdo->query($sql)->fetch();
if (empty($row)) {
    header('Location: bar_type_list.php');
    exit; # 結束 php 程式 
}
?>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form name="form1" method="post" onsubmit="sendForm(event)">
                            <div class="mb-3">
                                <label class="form-label">bar_type_id</label>
                                <input type="text" class="form-control" disabled value="<?= $row['bar_type_id'] ?>">
                                <div class="form-text"></div>
                            </div>
                            <input type="hidden" name="bar_type_id" value="<?= $row['bar_type_id'] ?>">
                            <div class="mb-3">
                                <label for="bar_type_name" class="form-label">bar_type_name</label>
                                <input type="text" class="form-control" id="bar_type_name" name="bar_type_name" value="<?= $row['bar_type_name'] ?>">
                                <div class="form-text"></div>
                            </div>
...
<!-- No Edit Modal -->
    ...
    let initialValues = {
        ...
        bar_type_name: document.form1.bar_type_name.value
    };
        ...
        // 檢查是否有更改
        let isChanged = initialValues.bar_type_id !== document.form1.bar_type_id.value ||
                initialValues.bar_type_name !== document.form1.bar_type_name.value;
